...
import com.orange.score.database.score.dao.LuohuMapper;
import com.orange.score.database.score.model.Luohu;
import com.orange.score.module.score.service.ILuohuService;
...
@Service
@Transactional
public class LuohuServiceImpl extends BaseService<Luohu> implements ILuohuService {

    @Autowired
    private LuohuMapper luohuMapper;

    @Autowired
    private IColumnJsonService iColumnJsonService;

    @Override
    public PageInfo<Luohu> selectByFilterAndPage(Luohu luohu, int pageNum, int pageSize) {
        PageHelper.startPage(pageNum, pageSize);
        List<Luohu> list = selectByFilter(luohu);
        return new PageInfo<>(list);
    }

    @Override
    public List<Luohu> selectByFilter(Luohu luohu) {
        Page<Luohu> tmp = SqlUtil.getLocalPage();
        SqlUtil.clearLocalPage();
        Condition condition = new Condition(Luohu.class);
        tk.mybatis.mapper.entity.Example.Criteria criteria = condition.createCriteria();
        if (luohu != null) {
            ColumnJson columnJson = new ColumnJson();
            columnJson.setTableName("T_LUOHU");
                ...
                if(StringUtils.isNotEmpty(columnJson.getSearchConf())){
                    for (Object o : jsonArray) {
                        ...
                        if (StringUtils.isNotEmpty(searchItem.getName())) {
                            Object value = MethodUtil.invokeGet(luohu, searchItem.getName());
                            if (value != null) {
        ...
        return luohuMapper.selectByCondition(condition);
    }
}
