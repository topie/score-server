...
//                SmsUtil.send(houseOther.getSelfPhone(), "系统提示：" + identityInfo.getName() + "，恭喜您已通过网上预审，下一步可以进行网上预约。");
    ...
    public Result disAgree(@RequestParam Integer id) throws IOException {
        ...
        if (identityInfo != null) {
            ...
            HouseOther houseOther = iHouseOtherService.findBy("identityInfoId", identityInfo.getId());
            if(identityInfo.getUnionApproveStatus2() != 3){
            ...
            for (Object o : jsonArray) {
                ...
                if (materials.size() > 0) {
                    ...
                    if(StringUtils.isNotEmpty(onlinePersonMaterial.getReason())){
                        onlinePersonMaterial.setReason(onlinePersonMaterial.getReason()+"<br/>"+strDate+"-公安预审："+reason);
                    }else {
                        onlinePersonMaterial.setReason(strDate+"-公安预审："+reason);
                    }
