...
import com.orange.score.database.score.dao.SmsSendConfigMapper;
...
@Service
@Transactional
public class SmsSendConfigServiceImpl extends BaseService<SmsSendConfig> implements ISmsSendConfigService {
    ...
    @Override
    public PageInfo<SmsSendConfig> selectByFilterAndPage(SmsSendConfig smsSendConfig, int pageNum, int pageSize) {
        PageHelper.startPage(pageNum, pageSize);
        List<SmsSendConfig> list = selectByFilter(smsSendConfig);
        return new PageInfo<>(list);
    }

    @Override
    public List<SmsSendConfig> selectByFilter(SmsSendConfig smsSendConfig) {
        Page<SmsSendConfig> tmp = SqlUtil.getLocalPage();
        SqlUtil.clearLocalPage();
        Condition condition = new Condition(SmsSendConfig.class);
        tk.mybatis.mapper.entity.Example.Criteria criteria = condition.createCriteria();
        if (smsSendConfig != null) {
            if (StringUtils.isNotEmpty(smsSendConfig.getStatusDictAlias())) {
                criteria.andEqualTo("statusDictAlias", smsSendConfig.getStatusDictAlias());
            }
            if (smsSendConfig.getStatusInt() != null) {
                criteria.andEqualTo("statusInt", smsSendConfig.getStatusInt());
            }
            ColumnJson columnJson = new ColumnJson();
            columnJson.setTableName("t_sms_send_config");
                ...
                if (StringUtils.isNotEmpty(columnJson.getSearchConf())) {
                    for (Object o : jsonArray) {
                        ...
                        if (StringUtils.isNotEmpty(searchItem.getName())) {
                            Object value = MethodUtil.invokeGet(smsSendConfig, searchItem.getName());
                            if (value != null) {
        ...
        return smsSendConfigMapper.selectByCondition(condition);
    }
}
